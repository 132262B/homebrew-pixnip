...
<img src="images/icon.png" width="128" alt="pixnip 아이콘 — 카메라 위로 고개를 내민 고양이">
...
이미지로도, 그 안의 글자로도. 다른 단축키 하나면 녹화가 시작되고, 멈추면 편집기가
기다립니다.
...
| **고정 영역**   | 틀을 두고 누를 때마다 같은 자리를. 바깥은 어두워지고, 틀 아래 앱은 그대로 쓰면서 |
| **화면 녹화**   | 같은 틀의 빨간 버튼. 창 하나만도 되고, 시스템 소리·마이크도 담기고, 멈추면 편집기가 열립니다 |
| **색상 추출**   | 확대경으로 픽셀을 집어 `#RRGGBB` 코드를 클립보드로                  |
| **텍스트 추출** | 드래그한 영역의 글자를 읽어 그대로 클립보드에                       |
| **화면에 그리기** | 강의·발표 중 화면 위에 펜·화살표·도형으로. 그림을 둔 채 아래 앱도 그대로 조작 |
...
`⌘⇧⌥R` 을 누르면 틀이 뜹니다. 시스템 스크린샷처럼 바깥은 어두워지고 틀 안만 밝게
남습니다. 둘레의 띠나 막대를 잡아 옮기고 모서리를 끌어 크기를 맞추면, 파란 버튼(또는
`↩`)을 누를 때마다 그 자리가 찍힙니다. 틀 안은 비어 있어 아래 앱을 그대로 쓸 수
있으니, 화면이 바뀔 때마다 같은 자리를 다시 찍는 데 좋습니다.

<img src="images/pinned.png" alt="pixnip 고정 영역 틀 — 어두워진 화면 가운데 밝게 남은 영역과, 아래의 셔터·녹화 버튼 막대" width="700">
...
소리도 원하면 함께 담깁니다. 설정 → 녹화에 시스템 소리(스피커로 나가는 소리)와
마이크(macOS 15 부터) 스위치가 있고, 각각 자기 트랙으로 들어가 나중에 따로 음량을
맞출 수 있습니다. 커서는 영상에 굽지 않습니다 — 어디로 움직이고 무엇을 클릭했는지를
기록해 두고 편집기가 다시 그립니다.

<img src="images/video-editor-ko.png" alt="pixnip 영상 편집기 — 배경화면 위의 녹화, 타임라인의 확대 구간과 클립, 아래의 파형 레이어" width="880">

## 그리고 보기 좋게 다듬습니다

녹화를 멈추면 어두운 영상 편집기가 열립니다. 마우스를 클릭한 곳은 잠시 확대되고
커서를 따라 화면이 움직이며, 커서는 부드럽게 다시 그려지고 클릭할 때마다 물결이
퍼집니다. 스크린샷과 같은 배경화면·그라디언트·단색 배경을 깝니다 — 프리셋 수십 개
또는 내 이미지. 영상은 타임라인의 클립으로 다룹니다 — 재생 위치에서 나누고(`C`), 지우고,
가장자리를 끌어 줄이고, 클립마다 속도를 0.5×~3× 로 정합니다. 화면·시스템 소리·마이크가
레이어로 나뉘어 파형을 보며 트랙마다 음량을 맞춘 뒤 MP4(H.264·HEVC)·WebM·GIF 로,
원본·1080p·720p, 30·60fps 로 내보냅니다.
