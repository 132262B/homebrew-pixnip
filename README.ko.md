...
`⌘⇧⌥V` 를 누르면 고정 영역 캡처와 같은 틀이 뜹니다. 원하는 자리로 끌어 놓고 크기를
맞추거나, 막대의 버튼을 씁니다 — 디스플레이 버튼은 화면 전체를, 창 버튼은 클릭한 창
하나만 녹화합니다. 다시 `⌘⇧⌥V`(또는 녹화 버튼)를 누르면 시작되어 테두리가 빨개지고
막대에 시간이 흐릅니다. 한 번 더 누르면 멈춥니다. 녹화하는 동안 틀 아래 앱은 그대로
쓸 수 있습니다.
